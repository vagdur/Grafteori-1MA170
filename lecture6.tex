...
\begin{xca}
  ...
  Is the stronger statement that in a graph G with distinct weights no two spanning trees can have the same weight also true?
\end{xca}
...
\begin{xca}\label{xca:implement_prim_kruskal}
  Use Prim's algorithm and Kruskal's algorithm to find a minimum spanning tree of the graph in Figure \ref{fig:new_exercise}.
  \begin{figure}
      \centering
      \includegraphics[width=0.7\textwidth]{graphics/L6_prim_kruskal_dijkstra/graph_exc6-7.png}
      \caption{Graph for Exercises \ref{xca:implement_prim_kruskal} and \ref{xca:implement_dijkstras}.}
      \label{fig:new_exercise}
  \end{figure}
\end{xca}

\begin{xca}\label{xca:implement_dijkstras}
  Implement Dijkstra's algorithm on the graph in Figure \ref{fig:new_exercise} using the vertex in the lower left corner as the initial vertex $v_0$.
\end{xca}
